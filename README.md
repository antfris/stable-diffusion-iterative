...
## usage
first modify and then run `run-iterations.ps1`.

edit `--prompt "Austrian alps"`, the first `--init-img cat.png`, then all other `--init-img`'s need to be the path of the previously generated image.

the script will generate the name of the next image using the `--name` attribute of the command. Make sure the `--name` attribute ascends from command to command.

to run the script type `.\run-iterations.ps1`.

## todo
make the script paths and name easily editable.
put in a loop.

# @basujindal's fork of stable diffusion update: v0.9
...
**original stable diffusion below**
